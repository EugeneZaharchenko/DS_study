...
def Model(n):
    S0=np.zeros((n))
    for i in range(n):
        S0[i]=26.01279575148775 + 0.100228697697014388 * i + -0.0001742478512703249 * i * i   # квадратична модель реального процесу
    return S0
...
# ----- модель виміру (квадратичний закон) з нормальним шумом + АНОМАЛЬНІ ВИМІРИ
def Model_NORM_AV(S0, SV, nAV, Q_AV):
    SV_AV = SV
    SSAV = np.random.normal(dm, (Q_AV * dsig), nAV)  # аномальна випадкова похибка з нормальним законом
    for i in range(nAV):
        k=int (SAV[i])
        SV_AV[k] = S0[k] + SSAV[i]        # аномальні виміри з рівномірно розподіленими номерами
    return SV_AV
...
if __name__ == '__main__':
    ...
    if (Data_mode == 1):
        ...
        # ----------------------------- Нормальні похибки ------------------------------------
        SV = Model_NORM(S, S0, n)  # модель тренду + нормальних помилок
        Plot_AV(S0, SV, 'квадратична модель + Норм. шум')
        Stat_characteristics_in(SV, 'Вибірка + Норм. шум')
        # ----------------------------- Аномальні похибки ------------------------------------
        SV_AV = Model_NORM_AV(S0, SV, nAV, Q_AV)  # модель тренду + нормальних помилок + АВ
